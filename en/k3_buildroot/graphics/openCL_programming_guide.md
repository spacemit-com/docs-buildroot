...
OpenCL (Open Computer Language) is an open, cross-platform parallel-computing framework by the Khronos Group.
It provides developers with a unified programming interface that enables applications to run across hardware platforms, such as CPUs, GPUs, DSPs, and other processors.
...
![opencl](./static/how_it_works.jpg)
...
err = clGetPlatformIds(num_platform, platform, NULL); // Retrieve platform ID
```
...
cl_context clCreateContext(
    ...
    void (CL_CALL_BACK *pfn_notify)(const char *errinfo, const void *private_info, size_t cb, void *user_data),  //  void (CL_CALL_BACK *pfn_notify)(const char *errinfo, const void *private_info, size_t cb, void *user_data),  // A user-defined callback function, paired with `user_data`, that reports any errors occurring during the context’s lifetime.
...
4. **Create kernel object:**
Finally, create kernel objects of type `cl_kernel`.
...
cl_mem clCreateBuffer(
    ...
    cl_int *errcode_ret) // Pointer for returning the error code; no error code is returned if it is Null
```
...
cl_int clEnqueueNDRangeKernel(
    ...
    cl_uint num_events_in_wait_list, // Number of events in wait list
    const cl_event *event_wait_list, // Array of events in wait list
...
cl_int clEnqueueReadBuffer(
    ...
    cl_mem buffer,  // Kernel to execute
...
Finally, use the `clRelease*` APIs to release all resources:
...
|-- CMakeLists.txt // Used for compilation
`-- README.md
...
add_executable(testDemo ${CMAKE_CURRENT_SOURCE_DIR}/testDemo.c) # Create an executable file 
...
### Using `printf` in a Kernel

Due to GPU hardware limitations, the maximum size of the `printf` buffer in OpenCL is about 360,000 bytes.
...
If the source code is imported through an external `xxx.cl` file, there is no need to add escape characters.
