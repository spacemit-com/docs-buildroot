...
It acts as a bridge between OpenGL ES rendering API and local window system.
...
4. **OpenGL ES Rendering Operation**
...
The `k1x-gpu-test` demo provides usage examples and wrappers for the K3 platform.
...
The current GPU DDK supports the latest version of **OpenGL ES 3.2**.
...
3. **Create Program Object**
    `glCreateProgram` is used to create a new OpenGL program object and returns a handle to it. A program object is a container used to store and manage an OpenGL program.
...
```bash
sudo apt update && sudo apt install gpu-test
```
...
```c
.
|-- CMakeLists.txt // CMake file, used for compilation and building
|-- README.md
|-- common
...
To add a new file named `testDemo.c` and compile it to generate an executable file named `testDemo`, you can modify the `CMakeList.txt` file as shown in the following example:
...
```bash
xxx/buildroot-sdk/package-src/gpu-test/vulkanDemo
```

In the **Bianbu** system, you can install `gpu-test` using the following command to obtain relevant demo:

```bash
sudo apt update && sudo apt install gpu-test
```

The compilation and running methods are the same as those for the OpenGL ES demo mentioned earlier. Please refer to [Compile & Run](#compile--run).