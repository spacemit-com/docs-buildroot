...
It is designed to abstract the differences in hardware encoding and decoding across multiple platforms and provide developers with a unified set of APIs.
...
- **MPP (Multimedia Processing Platform)**
  The architecture of the multimedia processing platform.

- **MPI (Multimedia Processing Interface)**
  The unified API provided by MPP for upper-layer access.

- **MPP AL**
  The abstraction layer, which unifies multimedia interfaces across different IPs, SoCs, and solutions.

- **Packet**
  A compressed video data unit, such as an H.264 or H.265 video stream. It is used before decoding and after encoding.

- **Frame**
  An uncompressed video data unit, such as a YUV420 image. It is used before encoding and after decoding.
...
- **VDEC**
  A video decoding submodule and its public APIs, mainly used to decode a compressed data stream (`Packet`) into a raw frame (`Frame`).
  
- **VENC**
  A video encoding submodule and its public APIs, mainly used to encode a raw frame (`RGB/YUV`) into a compressed video stream (`Packet`).

- **G2D**
  A 2D graphics processing submodule and its public APIs, mainly used for image processing operations.

- **BIND System**
  Supports dynamic binding across multiple modules.

- **AL (Abstract Layer)**
  A multi-platform support layer that abstracts away hardware differences.

- **VI**
  A video input submodule and its public APIs. Currently, it supports only file input and standard V4L2 input.
  
- **VO**
  A video output submodule and its public APIs. Currently, it supports only file output and SDL2 video output.

Parts not yet included:

- **AI/AO**
  Audio input and output. It uses the standard path: `pipewire -> alsa-lib -> alsa driver`.

- **AENC/ADEC**
  Not supported yet. Software implementations in GStreamer or FFmpeg are recommended.

### 1.3 Configuration

#### 1.3.1 Debugging Configuration

|       Configuration item               | Default Value                       | Functionality                                        |
| ----------------------------------- | ------------------------- | ------------------------------------------- |
...
| **MPP\_PRINT\_UNFREE\_FRAME**       | `0`                         | When set to `1`, it will print the allocation and release status of frames in real time.                    |
...
The source code of MPP is located at:

```shell
buildroot-sdk/package-src/mpp
```

#### 1.4.2 Code Compilation

In the Buildroot solution, compilation is enabled by default. To rebuild after modifying the code, run:
...
```shell
|-- al                               // AL(Abstract Layer): Code for interfacing with functional modules or drivers of various platforms.
|   |-- CMakeLists.txt
|   |-- include
...
|   |   |   |-- CMakeLists.txt
|   |   |   |-- linlonv5v7           // Interface with V4L2 encoding and decoding (K1 & K3)
|   |   |   |   |-- include
|   |   |   |   |   |-- linlonv5v7_buffer.h
|   |   |   |   |   |-- linlonv5v7_codec.h
...
- **MPP AL (Abstract layer)**  
  Abstracts differences across platforms and hardware to provide a unified invocation model.

From the functionality perspective, it is divided into:

- **MPI**  
  External API interface layer.
- **MPP AL**  
  Multi-platform hardware abstraction layer.
- **TESTS**  
  Test programs and test streams for function verification.
- **UTILS**  
  A utility package that implements basic functionality, including packet and frame management, log output, and environment variable access.
- **SYS**  
  Implements dynamic plugin loading and the BIND system.
...
This enumeration defines the supported plugin types, and plugins can be selected through it. `AUTO` indicates that plugins are selected according to the default priority, although that logic is not yet fully refined. `CODEC_V4L2_LINLONV5V7` is generally selected for encoding and decoding on K1.
...
This enumeration defines the supported encoding formats, including all formats supported by decoders and encoders. Each codec may support only a subset of these types. For example, OpenH264 supports only H.264 encoding and decoding.
...
Note: Each codec defines its own format types, so format conversion is required. The following example shows format mapping for `ffmpeg`:
...
This enumeration defines the supported pixel formats, including all formats supported by decoders and encoders. Each codec may support only a subset of these types.
...
Note: Each codec defines its own format types, so format mapping is required. The following example shows format mapping for `ffmpeg`:
...
Base data structure. `MppPacket` and `MppFrame` inherit from `MppData`.
...
The video decoder context is created and initialized through `VDEC_CreateChannel` and `VDEC_Init`.
...
Definition of the node operation set.
...
This enumeration defines node types.
...
struct _MppPacket {
   ...
   * parent class
   */
  MppData eBaseData;              // MppData base
...
| ----------------------- | ------------------ | ------------------------------------------------------------ | ------------------------- |
| VDEC_CreateChannel      | Create a decoder channel       | None                                                           | `MppVdecCtx*`: decoder context |
| VDEC_Init               | Initialize the decoder       | `MppVdecCtx *ctx`: decoder context                               | `0`: success; <br>non-zero: error code       |
| VDEC_SetParam           | Set decoder parameters    | `MppVdecCtx *ctx`: decoder context                                 | `0`: success; <br>non-zero: error code       |
| VDEC_GetParam           | Get decoder parameters     | `MppVdecCtx *ctx`: decoder context; `MppVdecPara **stVdecPara`: parameter structure | `0`: success; <br>non-zero: error code |
| VDEC_GetDefaultParam    | Get default parameters | `MppVdecCtx *ctx`: decoder context                                 | `0`: success; <br>non-zero: error code       |
| VDEC_Decode             | Send a bitstream to the decoder   | `MppVdecCtx *ctx`: decoder context; `MppData *sink_data`: buffer     | `0`: success; <br>non-zero: error code     |
| VDEC_RequestOutputFrame | Request a decoded output frame         | `MppVdecCtx *ctx`: decoder context; `MppData *src_data`: decoded output frame | `0`: success; <br>non-zero: error code       |
| VDEC_ReturnOutputFrame  | Return a decoded output frame     | `MppVdecCtx *ctx`: decoder context; `MppData *src_data`: decoded output frame | `0`: success; <br>non-zero: error code      |
| VDEC_DestroyChannel     | Destroy the decoder channel        | `MppVdecCtx *ctx`: decoder context                                 | `0`: success; <br>non-zero: error code    |
| VDEC_ResetChannel       | Reset the decoder         | `MppVdecCtx *ctx`: decoder context                               | `0`: success; <br>non-zero: error code       |

### 5.2 VENC

| Interface                       | Description                 | Parameter                                                         | Return Value                    |
| -------------------------- | -------------------- | ------------------------------------------------------------ | ------------------------- |
| VENC_CreateChannel         | Create an encoder channel           | None                                                         | `MppVencCtx*`: encoder context |
| VENC_Init                  | Initialize the encoder        | `MppVencCtx *ctx`: encoder context                                | `0`: success; <br>non-zero: error code    |
| VENC_SetParam              | Set encoder parameters      | `MppVencCtx *ctx`: encoder context; `MppVencPara *para`: encoder parameters  | `0`: success; <br>non-zero: error code    |
| VENC_GetParam              | Get encoder parameters       | `MppVencCtx *ctx`: encoder context; `MppVencPara *para`: encoder parameters  | `0`: success; <br>non-zero: error code      |
| VENC_SendInputFrame        | Send an input frame to the encoder        | `MppVencCtx *ctx`: encoder context; `MppData *sink_data`: input frame     | `0`: success; <br>non-zero: error code       |
| VENC_ReturnInputFrame      | Return an input frame to the encoder     | `MppVencCtx *ctx`: encoder context; `MppData *sink_data`: input frame     | `0`: success; <br>non-zero: error code     |
| VENC_GetOutputStreamBuffer | Get the encoded output stream buffer      | `MppVencCtx *ctx`: encoder context; `MppData *src_data`: encoded bitstream | `0`: success; <br>non-zero: error code      |
| VENC_DestroyChannel        | Destroy the encoder channel          | `MppVencCtx *ctx`: encoder context                              | `0`: success; <br>non-zero: error code       |
| VENC_ResetChannel          | Reset the encoder          | `MppVencCtx *ctx`: encoder context                                | `0`: success; <br>non-zero: error code    |
| VENC_Flush                 | Flush the internal encoder buffer | `MppVencCtx *ctx`: encoder context                                | `0`: success; <br>non-zero: error code      |

### 5.3 G2D (Under Improvement)

| Interface                    | Description           | Parameter                                                  | Return Value                |
| ---------------------- | -------------- | ------------------------------------------------------ | --------------------- |
| G2D_CreateChannel      | Create a G2D channel        | None                                                     | `MppG2dCtx*`: G2D context |
| G2D_Init               | Initialize G2D    | `MppG2dCtx *ctx`: G2D context                               | `0`: success; non-zero: error code   |
| G2D_SetParam           | Set G2D parameters    | `MppG2dCtx *ctx`: G2D context; `MppG2dPara *para`: G2D parameters     | `0`: success; <br>non-zero: error code   |
| G2D_GetParam           | Get G2D parameters    | `MppG2dCtx *ctx`: G2D context; `MppG2dPara *para`: G2D parameters     | `0`: success; <br>non-zero: error code   |
| G2D_SendInputFrame     | Send an input frame   | `MppG2dCtx *ctx`: G2D context; `MppData *sink_data`: frame to be processed  | `0`: success; <br>non-zero: error code  |
| G2D_ReturnInputFrame   | Return an input frame | `MppG2dCtx *ctx`: G2D context; `MppData *sink_data`: frame to be processed | `0`: success; <br>non-zero: error code |
| G2D_RequestOutputFrame | Request an output frame | `MppG2dCtx *ctx`: G2D context; `MppData *src_data`: processed frame | `0`: success; <br>non-zero: error code |
| G2D_ReturnOutputFrame  | Release a processed frame | `MppG2dCtx *ctx`: G2D context; `MppData *src_data`: processed frame | `0`: success; <br>non-zero: error code   |
| G2D_DestoryChannel     | Destroy the G2D channel        | `MppG2dCtx *ctx`: G2D context                                | `0`: success; <br>non-zero: error code   |

### 5.4 VI

| Interface                  | Description         | Parameter                                               | Return Value              |
| -------------------- | ------------ | --------------------------------------------------- | ------------------- |
| VI_CreateChannel     | Create a VI channel      | None                                                  | `MppViCtx*`: VI context |
| VI_Init              | Initialize VI     | `MppViCtx *ctx`: VI context                            | `0`: success; <br>non-zero: error code |
| VI_SetParam          | Set VI parameters   | `MppViCtx *ctx`: VI context; `MppViPara *para`: VI parameters     | `0`: success; <br>non-zero: error code |
| VI_GetParam          | Get VI parameters   | `MppViCtx *ctx`: VI context; `MppViPara *para`: VI parameters     | `0`: success; <br>non-zero: error code |
| VI_RequestOutputData | Get input data | `MppViCtx *ctx`: VI context; `MppData *src_data`: input data | `0`: success; <br>non-zero: error code |
| VI_ReturnOutputData  | Release input data | `MppViCtx *ctx`: VI context; `MppData *src_data`: input data | `0`: success; <br>non-zero: error code |
| VI_DestoryChannel    | Destroy the VI channel       | `MppViCtx *ctx`: VI context                             | `0`: success; <br>non-zero: error code |

### 5.5 VO

| Interface              | Description       | Parameter                                                 | Return Value                    |
| ----------------- | ---------- | ---------------------------------------------------- | -------------------------- |
| VO_CreateChannel  | Create a VO channel    | None                                                   | `MppVoCtx *`: VO context |
| VO_Init           | Initialize VO  | `MppVoCtx *ctx`: VO context                          | `0`: success; <br>non-zero: error code        |
| VO_SetParam       | Set VO parameters | `MppVoCtx *ctx`: VO context; `MppVoPara *para`: VO parameters      | `0`: success; <br>non-zero: error code        |
| VO_GetParam       | Get VO parameters | `MppVoCtx *ctx`: VO context; `MppVoPara **para`: VO parameters    | `0`: success; <br>non-zero: error code      |
| VO_Process        | Output data   | `MppVoCtx *ctx`: VO context; `MppData *sink_data`: output data | `0`: success; <br>non-zero: error code        |
| VO_DestoryChannel | Destroy the VO channel     | `MppVoCtx *ctx`: VO context                              | `0`: success; <br>non-zero: error code        |

### 5.6 SYS

| Interface            | Description                       | Parameter                                                        | Return Value                         |
| -------------- | -------------------------- | ------------------------------------------------------------ | ------------------------------ |
| SYS_GetVersion | Get the MPP version              | `MppVersion *version`: MPP version                               | `0`: success; <br>non-zero: error code            |
| SYS_CreateFlow | Create a BIND flow             | None                                                          | `MppProcessFlowCtx*`: flow context |
| SYS_CreateNode | Create a BIND node      | `MppProcessNodeType type`: node type                            | `MppProcessNode*`: node context    |
| SYS_Init       | Initialize a BIND flow           | `MppProcessFlowCtx *ctx`: flow context                           | None                            |
| SYS_Destory    | Destroy a BIND flow             | `MppProcessFlowCtx *ctx`: flow context                           | None                            |
| SYS_Bind       | Bind a data source to a data receiver       | `MppProcessFlowCtx *ctx`: flow context; `MppProcessNode *src_ctx`: data source; `MppProcessNode *sink_ctx`: data receiver | `0`: success; <br>non-zero: error code           |
| SYS_UnBind     | Unbind all data sources and data receivers | `MppProcessFlowCtx *ctx`: flow context                           | None                            |
| SYS_Handledata | Process data                   | `MppProcessFlowCtx *ctx`: flow context; `MppData *sink_data`: data to be processed | None                             |
| SYS_Getresult  | Get the result                  | `MppProcessFlowCtx *ctx`: flow context; `MppData *src_data`: processed data | None                            |
...
Use the LINLONV5V7 decoder to decode the H.264 bitstream (`input.264`) from the file into NV12-formatted `output.yuv`, and save the result as follows:
...
The test program flow is straightforward and is not described in detail here. Refer to the source code at:
...
Use the LINLONV5V7 encoder to encode the 1280 × 720 NV12-formatted `input.yuv` file into an H.264 bitstream (`output.264`), and save the result as follows:
...
The code flow of the test program is relatively simple and is not described in detail here. Refer to the source code at:

```shell
mpp/test/vi_file_venc_sync_userptr_vo_file_test.c
```

### 6.3 Decode-Encode Test (`vi_file_vdec_venc_sync_userptr_vo_file_test`)

```shell
VI(file) --> VDEC(linlonv5v7) --> VENC(linlonv5v7) --> VO(file)
```

#### 6.3.1 Test Program Usage
...
Use the LINLONV5V7 codec to decode the 1280 × 720 H.264 bitstream (`input.264`) from the file into NV12 format, and then re-encode it into an H.264 bitstream (`output.264`), as follows:
...
## 7. Hardware Decoding and Verification Methods

### 7.1 Bianbu Desktop System

#### 7.1.1 `mpv` Player

`mpv` supports hardware decoding for formats such as H.264, HEVC, VP8, VP9, MJPEG, and MPEG4, with support up to 4K@60 fps. The verification methods are as follows:

- In the desktop system, locate the video file, right-click it, and select **Play with mpv**.
- In the terminal, play the video from the command line.
...
`totem` supports hardware decoding for formats such as H.264, HEVC, VP8, VP9, and MJPEG, with support up to 4K@60 fps. The verification methods are as follows:

- In the desktop system, locate the video file, right-click it, and select **Play with totem**.
- In the terminal, play the video from the command line.
...
chromium supports hardware decoding for formats such as H.264 and HEVC, with support up to 4K@60 fps. The verification methods are as follows:

- Open chromium and play a video from Bilibili.
- Open chromium and play a video from Youku.
- Open chromium and play a video from Sina Sports.
- Open chromium and play a video from another website.

#### 7.1.4 `ffplay` Command Line
...
```
ffplay -codec:v h264_stcodec xxx.mp4 (HEVC video codec)
ffplay -codec:v hevc_stcodec xxx.mp4 (HEVC video codec)
...
```
