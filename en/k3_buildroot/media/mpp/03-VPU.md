...
### 1.1 Decoding Specifications (4 cores @ 1 GHz)
...
`k3x-vpu-test` is a set of programs used to test the functionality and performance of the K3 VPU. It also serves as a reference for application development that interfaces with the VPU for hardware encoding and decoding.

### 2.1 Installation Instructions

#### 2.1.1 Bianbu Desktop System

`k3x-vpu-test` is included in the software repository and can be installed with `apt`.

```shell
sudo apt update
sudo apt install k3x-vpu-test
```

#### 2.1.2 Buildroot System

There are two ways to integrate `k3x-vpu-test` into the system:

- Enable the `k3x-vpu-test` build and integration option when building the image. This option is enabled by default, so the image includes the related `k3x-vpu-test` programs.
- If this option is disabled, `k3x-vpu-test` can be compiled manually, and the generated binaries can be copied to `/usr/bin/` in the target system. The binaries are listed below.

### 2.2 Usage Instructions

The `k3x-vpu-test` suite includes the following programs:

- **`mvx_decoder`**: Used for single-channel video bitstream decoding tests.
- **`mvx_decoder_multi`**: Used for multi-channel video bitstream decoding tests. Multi-channel video bitstreams must use the same source video.
- **`mvx_encoder`**: Used for single-channel video bitstream encoding tests.
- **`mvx_encoder_multi`**: Used for multi-channel YUV stream encoding tests. Multi-channel YUV streams must use the same source stream.
- **`mvx_logd`**: Used to capture firmware logs for issue analysis and debugging.
...
```shell
// Decode the H.264 video stream (with start code) from input.264 to output.yuv using 4 concurrent decoding sessions.
// The default input encoding format is H.264.
// The default output YUV pixel format is YUV420P.
mvx_decoder_multi -n 4 /mnt/streams/input.264 /mnt/test/output.yuv

// Decode the HEVC video stream (with start code) from input.265 to output.yuv using 4 concurrent decoding sessions. The pixel format of output.yuv is NV21.
...
```shell
// Encode the YUV420P stream from input.yuv to an H.264 video stream (with start code) in output.264.
// The default input YUV pixel format is YUV420P.
// The default output video encoding format is H.264.
mvx_encoder -f raw -w 1280 -h 720 /mnt/streams/input.yuv /mnt/test/output.264

// Encode the NV21 stream from input.yuv to an HEVC video stream (with start code) in output.265.
...
```shell
// Encode the YUV420P stream from input.yuv into an H.264 video stream (with start code) in output.264 using 4 concurrent encoding sessions.
mvx_encoder_multi -n 4 -w 1280 -h 720 /mnt/streams/input.yuv /mnt/test/output.264

// Encode the NV21 stream from input.yuv into an HEVC video stream (with start code) in output.265 using 4 concurrent encoding sessions.
...
```shell
// Mount debugfs to save firmware logs to a file
mount -t debugfs none /sys/kernel/debug
// Clear logs
mvx_logd -C
// Set the log save path and format
...
The `k3x-vpu-test` source code is located at:

```shell
package-src/k3x-vpu-test
```
...
### 2.4 Compilation Instructions

**Bianbu Desktop System**

```shell
cd k3x-vpu-test
sudo apt-get build-dep k3x-vpu-test    #install dependencies
...
Add logs with `printf` or `fprintf`, then recompile and reinstall the program.

#### 2.5.2 Successful Decoding Logs
...
Stream on 9
// After receiving the resolution change message, close the output stream, reallocate the output buffer, and reopen the output stream.
...
Total size 26265600
Closing fd 5.
// Decoding completed. Output information is printed, and the program exits.
```
...
#### 2.5.3 Successful Encoding Logs
