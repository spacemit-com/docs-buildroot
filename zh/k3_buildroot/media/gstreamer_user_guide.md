...
在编译系统镜像时，开启 GStreamer 及相关插件的编译集成选项。若系统镜像中已使能 GStreamer 软件包，则可直接使用相关命令；若未使能，则需要在 Buildroot 配置中打开对应软件包后重新编译系统镜像。
...
- `BR2_PACKAGE_GSTREAMER1`：GStreamer 框架和基础库；
- `BR2_PACKAGE_GST1_PLUGINS_BASE`：基础插件集合；
- `BR2_PACKAGE_GST1_PLUGINS_GOOD`：成熟稳定的插件集合；
- `BR2_PACKAGE_GST1_PLUGINS_BAD`：包含部分平台相关或仍在开发中的插件，SpacemiT 相关插件通常在该集合中；
- `BR2_PACKAGE_GST1_PLUGINS_UGLY`：包含有 license 限制的插件集合；
- `BR2_PACKAGE_GST1_LIBAV`：基于 libav/ffmpeg 的 GStreamer 插件。
...
- UVC 摄像头的信息可以通过 `v4l2-ctl` 命令进行获取。
  ...
  **Buildroot 系统**
  ...
  通过以上命令可以获取摄像头支持的像素格式、分辨率、帧率，以及对应的 video capture 节点信息。
...
如果需要使用真实摄像头作为输入源，也可以将 `videotestsrc` 替换为 `v4l2src`，例如：
