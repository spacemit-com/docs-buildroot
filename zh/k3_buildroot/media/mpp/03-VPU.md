...
`k3x-vpu-test` 是一套用于测试和验证 K3 芯片 VPU 性能的程序集合，也可以作为客户开发基于 VPU 硬件编解码的应用的参考。

### 2.1 安装说明

#### 2.1.1 Bianbu 桌面系统

`k3x-vpu-test` 已集成在软件源中，可直接通过 `apt` 安装：
...
有两种方法将 `k3x-vpu-test` 集成到系统中：

- 在编译系统镜像时，开启 `k3x-vpu-test` 的编译集成选项（默认已开启），这样，编译的系统镜像中默认就包含 `k3x-vpu-test` 相关的测试程序;
- 若未开启此选项，则需手动编译 `k3x-vpu-test`，将生成的二进制文件复制到系统 `/usr/bin/` 目录下使用。具体包含的二进制文件在下文说明。

### 2.2 使用说明

`k3x-vpu-test` 的测试程序集中主要包含以下测试程序：
...
```shell
cd k3x-vpu-test
sudo apt-get build-dep k3x-vpu-test    #安装依赖
