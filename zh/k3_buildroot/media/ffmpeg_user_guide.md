...
K3 平台上的 FFmpeg 已对接平台硬件编解码器，插件名称采用“编解码格式_stcodec”形式。当前 K3 平台上的 FFmpeg 支持的编解码插件如下：
...
- 若命令中未显式指定 `-c:v xxx_stcodec`，FFmpeg 会自动选择软件编解码器。
...
在 Linux 环境下，可通过 V4L2 设备节点采集摄像头数据。使用前需要先确认设备节点及其格式能力。
...
参数说明：`-codec:v h264_stcodec` 指定使用 H.264 硬件解码器，可按实际输入格式替换为对应的 `stcodec` 解码器。

## 常见问题

### 如何确认当前使用的是硬件编解码器

在命令行中显式指定 `-c:v xxx_stcodec`，并结合以下命令确认 FFmpeg 是否包含对应编解码器：
