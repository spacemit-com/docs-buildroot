...
| :-----| :----: | :----: |
| Samsung 970 EVO Plus | 2264 MB/s | 3293 MB/s |
...
**注意：** K3 默认 defconfig 中 `SWIOTLB` 被条件排除（`select SWIOTLB if MMU && !SOC_SPACEMIT_K3`），需修改 `arch/riscv/Kconfig` 去掉该排除条件：

```diff
- select SWIOTLB if MMU && !SOC_SPACEMIT_K3
