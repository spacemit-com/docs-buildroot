...
1. 当前 **K3 与 K1 共用同一个 GPIO 驱动**：`gpio-spacemit-k1.c`
2. 这个驱动是 **新的标准化驱动**，同时支持 `spacemit,k1-gpio` 和 `spacemit,k3-gpio`
3. 它**不是** K1 旧文档中描述的历史 GPIO 驱动实现，写 K3 文档时已经以当前内核中的新驱动为准
...
- `gpio-ranges`：建立 GPIO bank/offset 与 pinctrl pin 的映射关系
...
K3 GPIO 通过 `gpio-ranges` 与 pinctrl 建立映射关系：
...
`gpio-ranges` 中每一组 5 个 cell 的含义如下：

```dts
<&pinctrl gpio-bank gpio-offset pin-number count>
```

分别表示：

1. `&pinctrl`：关联到哪个 pinctrl 控制器
2. `gpio-bank`：GPIO 控制器中的 bank 编号
3. `gpio-offset`：该 bank 内起始 GPIO offset
4. `pin-number`：pinctrl 中对应的起始 pin 编号
5. `count`：连续映射的 GPIO / pin 个数
...
- bank1 offset0~31 ↔ pin32~63

因此当前 K3 默认配置下，`gpio-ranges` 表示当前 SoC 默认只把 **GPIO_00 ~ GPIO_127** 这一段 pin 建立为 GPIO 控制器可见的 GPIO 范围，对应关系是：

- bank0 offset 0~31 对应 pin 0~31
- bank1 offset 0~31 对应 pin 32~63
- bank2 offset 0~31 对应 pin 64~95
- bank3 offset 0~31 对应 pin 96~127

也就是说，`gpio-ranges` 并不是固定只能这样写，它本质上描述的是：

- **GPIO 控制器中的某一段 GPIO 编号**
- 映射到 **pinctrl 中哪一段 pin 编号**

因此如果希望把默认不在 `GPIO_00 ~ GPIO_127` 范围内的 pin，当作 GPIO 使用，也需要同步调整 `gpio-ranges` 映射关系。
...
如果客户希望把 `EMMC_D0` 当作 GPIO 使用，并且希望它对应 **gpio 32**，那么 `gpio-ranges` 中就应该增加或修改一段映射，使 **GPIO bank1 offset0** 对应到 **pin 153**。

因为：

- gpio 32 = bank1 offset0
- `EMMC_D0` 的 pin number = 153
...
- `&pinctrl 1 0 153 1`
  - bank1 offset0 ↔ pin153 (`EMMC_D0`)
  - 也就是 **gpio 32 ↔ EMMC_D0**

- `&pinctrl 1 1 33 31`
  - bank1 剩余的 offset1~31 继续映射回原来的 pin33~63
  - 这样 bank1 的 32 个 GPIO 编号仍然完整

这样设备节点中如果写：

```dts
some-gpios = <&gpio 1 0 GPIO_ACTIVE_HIGH>;
```

其底层实际对应的就可以是 `EMMC_D0` 这根 pin。

因此在板级定制时，**不仅要在 pinctrl 中把该 pin 切到 GPIO 功能，还要保证 gpio-ranges 对 GPIO 编号和 pin 编号的映射是正确的**。
...
虽然文件名带 `k1`，但对 K3 同样适用，因为 K1/K3 共享同一套 GPIO IP，当前 Linux 也共用同一个新驱动。
