...
- 通过 Gate 为各模块提供时钟开关；
- 与 Reset 控制一起，为各模块提供完整的 “时钟 + 复位” 资源。
...
一般为了方便使用，各厂商会根据时钟树设计，实现一些新的时钟类型，以便使用。
...
|   |-- ccu_ddn.h                 # DDN 时钟头文件
|   |-- ccu-k1.c                  # K1 时钟控制单元驱动
|   |-- ccu-k3.c                  # K3 时钟控制单元驱动
|   |-- Kconfig                   # 配置文件
|   `-- Makefile                  # 编译makefile
|-- drivers/reset/
|   `-- reset-spacemit.c          # K1/K3 复位控制单元驱动
|-- include/dt-bindings/clock/
|   |-- spacemit,k1-syscon.h      # K1 clock/reset ID 定义
|   `-- spacemit,k3-syscon.h      # K3 clock/reset ID 定义
|-- include/soc/spacemit
|   |-- k1-syscon.h               # K1 Clock 和 Reset 寄存器定义
...
- PLL 类型，锁相环类型（包含 PLL 和 PLLA 两种类型，K3 使用 PLLA 类型；K1 使用标准 PLL 类型）
...
K3 时钟控制器按不同地址域分成了多个 clock controller，每个 clock controller 负责管理一个地址域内的时钟。
K3 clock controller 的 DTS 配置如下：
...
每个时钟控制器节点通过 `#reset-cells = <1>` 属性声明自己为 Reset Provider。各个设备节点通过 `resets` 和 `reset-names` 属性引用对应的 Reset 资源。

**示例**：CAN 设备的 Clock/Reset 配置
...
K3 Clock/Reset Provider 在 `k3.dtsi` 里已经定义好，板级 DTS 一般不需要重新创建 Provider，这部分一般没有什么工作。

### Consumer

Clock/Reset Consumer一般需要完成以下工作：

- 找到正确的 Clock/Reset Provider 和 Clock/Reset ID，在DTS节点中配置 `clocks` 和 `resets`；
- 在 DTS 节点中配置 `clock-names`，`reset-names` ：
  不同设备的 `clock-names`、`reset-names` 会有区别，具体看 clock/reset 的用途，也可以根据实际的情况自定义，主要是驱动获取句柄时需要使用，`clock-names` 常见的取值有`func`、`bus`、`clk`、`gate`等；
- 必要时，在 DTS 节点中配置 `assigned-clocks` / `assigned-clock-parents` / `assigned-clock-rates`；
- 设备驱动中通过接口获取 Clock 和 Reset 句柄，并进行 Clock 和 Reset 相关的操作。

大多数 K3 外设 DTS 节点写法都类似：
...
```
Clock/Reset Provider 和 Clock/Reset ID 可以通过ID的前缀来对应，具体对应关系如下：

| Clock/Reset id前缀 | 前缀对应的 Provider 节点 |
|---------------------|-----------------------|
| CLK_PLL_       | pll                   |
| CLK/RESET_MPMU_     | syscon_mpmu           |
| CLK/RESET_APBC_     | syscon_apbc           |
| CLK/RESET_APMU_     | syscon_apmu           |
| CLK/RESET_DCIU_      | syscon_dciu           |
| CLK/RESET_RCPU_SYSCTRL_ | syscon_rcpu_sysctrl  |
| CLK/RESET_RCPU_UARTCTRL_ | syscon_rcpu_uartctrl |
| CLK/RESET_RCPU_I2SCTRL_ | syscon_rcpu_i2sctrl  |
| CLK/RESET_RCPU_SPICTRL_ | syscon_rcpu_spictrl  |
| CLK/RESET_RCPU_I2CCTRL_ | syscon_rcpu_i2cctrl  |
| CLK/RESET_RPMU_     | syscon_rpmu           |
| CLK/RESET_RCPU_PWMCTRL_ | syscon_rcpu_pwmctrl  |
| CLK/RESET_APBC2_    | syscon_apbc2          |
...
以`can0`为例：
can0的Clock ID 有两个：CLK_APBC_CAN0 和 CLK_APBC_CAN0_BUS，分别对应func clk和bus clk，前缀CLK_APBC_对应 Provider 为syscon_apbc，所以其DTS 配置如下：
...
以`can0`为例：
can0 的 Reset ID 一个：RESET_APBC_CAN0，前缀 RESET_APBC_ 对应 Provider 为 syscon_apbc，所以其 DTS 配置如下：
...
- DTS 配置是否正确：`clocks` `resets` 是否配置，Provider 和 ID 是否正确；`clock-names` `reset-names` 顺序对不对；
- 驱动里实际 `devm_clk_get()` 请求的名字是什么，是否有报错，例如 `failed to get clk` `failed to enable clk` `failed to deassert reset`。
- 时钟树是否正常：通过 debugfs 看时钟树，确认时钟是否 enable，父时钟是谁，频率是不是预期的值；
- 驱动是否正确释放复位；
- 是否还有其他依赖没满足，比如 pinctrl / power-domain / regulator 等；
