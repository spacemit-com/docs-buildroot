...
4. `PWM_POLARITY_NORMAL`：PWM 极性

不过需要注意，驱动 `pwm-pxa.c` 中 `pxa_pwm_apply()` 明确限制：
...
K3 的 PWM 驱动并不是单独的新驱动，而是继续使用 `drivers/pwm/pwm-pxa.c` 这套框架，并通过 `spacemit,k1-pwm` compatible 和平台属性进行了扩展适配。写 K3 PWM 文档时，需要重点关注以下几点：

- **驱动文件仍然是 `pwm-pxa.c`**
- **K3 dts compatible 使用 `spacemit,k1-pwm`, `marvell,pxa910-pwm`**
- **dt-binding 使用 `marvell,pxa-pwm.yaml`**
- **`#pwm-cells = <3>` 是当前 SpacemiT 平台的关键约束**
- **实际输出还依赖 `k3-pinctrl.dtsi` 中的 PWM/RPWM 管脚复用配置**
- **驱动仅支持正常极性，并带有 `k1,pwm-disable-fd` 平台扩展行为**
