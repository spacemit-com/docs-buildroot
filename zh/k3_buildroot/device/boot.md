...
> **引入 TEE 后的固件布局**
>
> 安全镜像在上述布局基础上增加了一段独立的 TEE 固件分区（MTD 介质为 `optee` 512K，GPT 介质为 4M），并相应调整了 OpenSBI 分区大小、U-Boot 起始偏移与 SPL 的固件加载顺序。部分启动介质（USB 烧录）不设独立分区，`optee.itb` 放在 `bootfs` 内。完整的分区表参见 [安全 / TEE 镜像构建与烧录说明](../security/tee_image.md) 的分区布局章节，地址锚点与域归属参见 [安全 / TEE 架构与集成指南](../security/tee_architecture.md) 的内存布局章节。
